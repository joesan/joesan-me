...
## How to CI?
This project uses GitHub Actions to do CI & CD. [Have a look here](https://github.com/joesan/joesan-me/blob/master/.github/workflows/main.yml) on how the project is built. It is a simple process where we first check for valid HTTP links across all the markdown files. If that is successful, I then check all the markdown content that is in draft status. I write them to a file in a separate branch. Finally, the project is built using Zola and the resulting HTML pages are published to 
